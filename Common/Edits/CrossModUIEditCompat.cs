...
namespace GvMod.Common.Edits
{
    class CrossModUIEditCompat
    {
        ...
        static Vector2 OpenTextPosition => new(Main.screenWidth / 2f, Main.screenHeight / 2f + ModContent.GetInstance<ClientConfig>().MrPlaguesButtonOffsetY);

        internal static void AddCharCreationDetour()
        {
            NoticeText = Language.GetText("Mods.GvMod.MrPlaguesCompatLine");
            ...
            if (self.CurrentState is not null && self.CurrentState.GetType().FullName is PlaguesCharUIName or OnePieceCharUIName)
            {
                ...
                if (clickingOnThis)
                {
                    ...
                    if (plr.GetModPlayer<SeptimaPlayer>().septima == null || 
                        plr.GetModPlayer<SeptimaPlayer>().septimaType == SeptimaType.None)
                        plr.GetModPlayer<SeptimaPlayer>().SetSeptima(SeptimaTemplates.GetRandSeptima());

                    Main.MenuUI.SetState(new SeptimaSelectionUI(plr, (evt, listeningElement) => Main.MenuUI.SetState(state)));
                }
