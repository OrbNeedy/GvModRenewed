...
namespace GvMod.Common.UI
{
    public class SPBar : UIState
    {
        ...
        public override void OnInitialize()
        {
            ...
            Asset<Texture2D> fillingTexture = ModContent.Request<Texture2D>("GvMod/Common/UI/SPBarFill");
            Asset<Texture2D> fullTexture = ModContent.Request<Texture2D>("GvMod/Common/UI/SPBarFull");
            ...
            for (int i = 0; i < barFilling.Length; i++)
            {
                ...
                barFrame[i] = new UIImage(ModContent.Request<Texture2D>("GvMod/Common/UI/SPBarFrame"));
                barFrame[i].Left.Set(0 + (30 * i), 0f);
                barFrame[i].Top.Set(0, 0f);
                barFrame[i].Width.Set(18, 0f);
                barFrame[i].Height.Set(32, 0f);

                barIcon[i] = new UIImage(ModContent.Request<Texture2D>("GvMod/Assets/UI/AzureStrikerSPBarIcon"));
        ...
        public override void Update(GameTime gameTime)
        {
            ...
            for (int i = 0; i < barFilling.Length; i++)
            {
                ...
                {
                    barFilling[i].SetFrame(new Rectangle(0, 2 + (int)(20 * barSP) - 20, 14, 20));
                    barFilled[i].Remove();
                    barIcon[i].Remove();
