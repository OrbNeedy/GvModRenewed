...
/*
	Taken directly from https://github.com/stormytuna/GrapplingHookAlternatives/blob/main/Common/RenderTargets/PlayerRenderTarget.cs
 */
        ...
        private void DrawPlayerTarget()
        {
            ...
            for (int i = 0; i < Main.maxPlayers; i++)
            {
                ...
                if (player.active && player.dye.Length > 0)
                {
                    ...
                    Main.PlayerRenderer.DrawPlayer(Main.Camera, player, getPositionOffset(i), player.fullRotation,
                        player.fullRotationOrigin, 0f);
