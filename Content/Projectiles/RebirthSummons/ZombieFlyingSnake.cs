...
{
    class ZombieFlyingSnake : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            float acceleration = 0.28f;
            float maxSpeed = 14;
            float drag = 0.12f;

            TargetNPC = ZombieBat.TargetingSystem(Projectile, TargetNPC, 320, false);
            ...
            if (hasTarget)
            {
                ...
                if (DashTimer % 50 == 0 && Projectile.Center != targetPosition)
                {
                    Projectile.velocity = Projectile.Center.DirectionTo(targetPosition) * 64;
                }
            }
            else
            {
        ...
        {
            if (Main._rand.NextBool(5))
            {
                target.AddBuff(BuffID.Venom, 180);
            }

            if (Main._rand.NextBool(12))
            {
                GorgonGazeBeam.PetrifyNPC(target, 0.3f, ignoreCooldown: true);
            }
